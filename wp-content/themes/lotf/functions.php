...
function twentyeleven_continue_reading_link() {
	return ' <a class="read-more" href="'. esc_url( get_permalink() ) . '">' . __( 'READ MORE <span class="meta-nav">&raquo;</span>', 'twentyeleven' ) . '</a>';
}
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
					<?php
						...
						printf( __( '%1$s', 'twentyeleven' ),
							sprintf( '<span class="post-author">%s</span>', get_comment_author_link() ),
...
function twentyeleven_posted_on() {
	printf( __( '<time class="entry-date" datetime="%3$s">%4$s</time>', 'twentyeleven' ),
...
function parent_paginate_links( $args = '' ) {
	$defaults = array(
		...
		'next_text' => __('<div class="divider"></div>OLDER POSTS'),
		'end_size' => 1,
		'mid_size' => 2,
		'type' => 'plain',
	...
	if ( $prev_next && $current && 1 < $current ) :
		...
		$link .= $add_fragment;
		$page_links[] = '<a class="prev page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $prev_text . '</a><div class="divider"></div>';
	endif;
	for ( $n = 1; $n <= $total; $n++ ) :
...
if ( function_exists( 'add_image_size' ) ) { 
	add_image_size( 'category-thumb', 420, 280, true ); //300 pixels wide (and unlimited height)
